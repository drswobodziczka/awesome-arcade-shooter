...
describe('GameOverScene Logic', () => {
  describe('Score Display', () => {
    ...
    it('should handle score of 0 (not use default)', () => {
      // Edge case: score is actually 0 (not undefined/null)
      const mockRegistryValue = 0;
      const displayedScore = mockRegistryValue ?? 0; // Should use ?? for proper 0 handling
  ...
  describe('Scene Transitions', () => {
    it('returns to MenuScene on restart', () => {
      // This would require full Phaser mock - deferred
      // Test logic: this.scene.start('MenuScene')
      const expectedScene = 'MenuScene';
      expect(expectedScene).toBe('MenuScene');
    });
  });
});
